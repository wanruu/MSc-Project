...
class BiLSTM(nn.Module):
    ...
    def forward(self, input_ids, lengths, labels=None):
        ...
        if labels is not None:
            f_input_ids = torch.reshape(input_ids, (-1,))
            f_logits = torch.reshape(logits, (-1, logits.size(-1)))
            f_logits[f_input_ids==0] = -100
            f_labels = torch.reshape(labels, (-1,))
            f_labels[f_input_ids==0] = -100
            loss = nn.CrossEntropyLoss()(f_logits, f_labels)
